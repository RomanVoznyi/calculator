...
it("check minus operation without arguments", () => {
  action("-", { ...state });
...
it("check minus operation with inputNumb and without expression", () => {
...
it("check minus operation if inputNumb = 'error'", () => {
  action("-", { ...state, inputNumb: "error" });
  expect(setExpression).not.toBeCalled();
  expect(setInputNumb).not.toBeCalled();
});

it("check minus operation with inputNumb and expression", () => {
  action("-", {
    ...state,
    inputNumb: "5",
    expression: {
