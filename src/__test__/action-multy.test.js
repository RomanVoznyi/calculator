...
it("check multiplication operation without arguments", () => {
  action("*", { ...state });
...
it("check multiplication operation with inputNumb and without expression", () => {
...
it("check multiplication operation if inputNumb = 'error'", () => {
  action("*", { ...state, inputNumb: "error" });
  expect(setExpression).not.toBeCalled();
  expect(setInputNumb).not.toBeCalled();
});

it("check multiplication operation with inputNumb and expression", () => {
  action("*", {
    ...state,
    inputNumb: "5",
    expression: {
